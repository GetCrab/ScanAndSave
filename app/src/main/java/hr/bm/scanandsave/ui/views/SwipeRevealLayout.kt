...
class SwipeRevealLayout : ViewGroup {
    ...
    override fun onLayout(changed: Boolean, l: Int, t: Int, r: Int, b: Int) {
        for (index in 0 until childCount) {
            ...
            val matchParentHeight: Boolean = childParams.height == LayoutParams.MATCH_PARENT ||
                    childParams.height == LayoutParams.FILL_PARENT
            val matchParentWidth: Boolean = childParams.width == LayoutParams.MATCH_PARENT ||
            ...
            Log.d("MYTAG", "childIndex: $index, left: $left, right: $right, marginEnd: " + child.paddingEnd + "  " + paddingEnd)
            when (mDragEdge) {
                DRAG_EDGE_RIGHT -> {
            ...
            Log.d("MYTAG", "childIndex: $index, left: $left, right: $right")
    ...
    private val mainOpenLeft: Int
        get() = when (mDragEdge) {
            DRAG_EDGE_LEFT -> mRectMainClose.left + mSecondaryView!!.width - mSecondaryView!!.paddingEnd
            DRAG_EDGE_RIGHT -> mRectMainClose.left - mSecondaryView!!.width + mSecondaryView!!.paddingEnd + 20
            else -> 0
        }
        ...
        override fun clampViewPositionHorizontal(child: View, left: Int, dx: Int): Int {
            return when (mDragEdge) {
                DRAG_EDGE_RIGHT -> left.coerceAtMost(mRectMainClose.left)
                    .coerceAtLeast(mRectMainClose.left - mSecondaryView!!.width + mSecondaryView!!.paddingEnd + 20)
                DRAG_EDGE_LEFT -> left.coerceAtMost(mRectMainClose.left + mSecondaryView!!.width - mSecondaryView!!.paddingEnd - 20)
